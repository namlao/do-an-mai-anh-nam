                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('display_name') is-invalid @enderror"
                                       ...
                                       placeholder="" value="{{ $role->display_name}}">
                                @error('display_name')
...
{{--                            </div>--}}
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description"
                                          ...
                                          rows="5" style="resize: none">{{ $role->description }}</textarea>
                                @error('description')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <select class="form-select form-permission" size="10" aria-label="size 10 select example"
                                    name="permission[]"
                                    {{--                                    style="height: 250px"--}}
                                    multiple>
                                @foreach($permissions as $permission)

                                    <option value="{{$permission->name}}" class="option-permission"
                                            style="text-transform: capitalize;padding: 0.7rem 1.2rem"
                                            @foreach($roleSelecteds as $roleSelected)
                                                @if($permission->name === $roleSelected)
                                                    selected
                                                @endif
                                            @endforeach
                                    >
